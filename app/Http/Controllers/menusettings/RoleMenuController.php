...
INNER JOIN menus on role_menu.menu_id=menus.id');
		return view("menusettings.rolemenu.index",['result'=>$result,'accessStatus'=>$accessStatus]);
	}
	public function create(){
		...
		if($accessStatus[2]==1){
			$roles=Role::all();
			$menus=Menu::all();
			return view("menusettings.rolemenu.create",['roles'=>$roles,'menus'=>$menus]);
		}else{
...
INNER JOIN menus on role_menu.menu_id=menus.id where role_menu.id=?',[$id]);
			...
			$menus=Menu::all();
			$aBean=$result[0];
			return view("menusettings.rolemenu.edit",['bean'=>$aBean,'roles'=>$roles,'menus'=>$menus]);
		}else{
			return redirect('rolemenu');
		}
	}
